...
void loadSprites() {
  ...
  for(int i = 0; i < game.get_playerList().at(0)->get_hand()->get_cards().size(); i++) {
    game.get_playerList().at(0)->get_hand()->get_cards().at(i)->set_sprite(al_load_bitmap(game.get_playerList().at(0)->get_hand()->get_cards().at(i)->get_spriteName().c_str()));
  }
}
...
int main() {
    ...
    while(1)
    {
        ...
        if(redraw && al_is_event_queue_empty(queue))
        {
            ...
            for (int i = 0; i < game.get_playerList().at(0)->get_hand()->get_cards().size(); i++)  {
              if (game.get_playerList().at(0)->get_hand()->get_cards().at(i)->get_sprite() != NULL) {
